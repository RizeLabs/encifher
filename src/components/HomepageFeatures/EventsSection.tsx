...
const ViewAllButton: React.FC<ViewAllButtonProps> = ({ className }) => {
  return (
    <Link
      to="/blog"
      className={`flex gap-[10px] hover:no-underline hover:text-white justify-center items-center hover:cursor-pointer hover:opacity-95 bg-primary-dark py-2 px-4 md:py-3 md:px-8 text-base md:text-xl font-semibold rounded-[60px] border-none text-white ${className}`}
    >
      <span>View All</span>
      <img
      ...
      <div className="w-screen lg:px-20 md:px-10 px-5 py-16 md:py-32 flex flex-col items-start bg-footer-bg gap-24 z-10" >
        <div className="max-w-[400px] md:w-full md:max-w-none mr-auto ml-auto">
          ...
          <div className="flex flex-wrap gap-8 justify-center " data-aos="fade-up">
            {recentPosts.slice(0, 3).map(({ content: BlogPostContent }) => (
              <BlogPostProvider
